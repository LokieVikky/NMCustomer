...
import 'package:namma_mechanics/models/garage/sub_models/ImagesModel.dart';
import 'package:namma_mechanics/models/garage/sub_models/MechanicServies.dart';
import 'package:namma_mechanics/models/garage/sub_models/addressModel.dart';
import 'package:namma_mechanics/models/garage/sub_models/contactsModel.dart';
import 'package:namma_mechanics/models/garage/sub_models/ownerDetailsModel.dart';
import 'package:namma_mechanics/models/garage/sub_models/timingModel.dart';
import 'package:namma_mechanics/models/orderModel.dart';
import 'package:namma_mechanics/models/ourUser.dart';
import 'package:namma_mechanics/services/database.dart';
import 'package:namma_mechanics/values/Constants.dart';
...
class CurrentState extends ChangeNotifier {
  ...
  List<OrderModel> pastOrders = List<OrderModel>();
  ...
  Future<String> verifyOtp(String input, context) async {
    String retVal = "error";
  ...
  Future<String> _checkUser(String uid, String input) async {
    ...
    try {
      var value = await dio.post(
          "https://namma_mechanics.herokuapp.com/loginRoutes/checkUser",
          data: {
            "phoneNumber": _inputText,
            "gid": uid,
          },
  ...
  // if the otp is verified show the user the success message and also update the stage of the order to 2
  Future<String> verifyOTP(String otp) async{
    String _retVal = "success";
    if(otp == orderData["otp"].toString()) {
      _retVal = "success";
      await _firestore.collection("currentOrder").doc(orderData["id"]).update({
        "otpVerified" : true,
      });
      orderData["otpVerified"] = true;
    }
    else{
      _retVal = "wrong otp";
    }
  ...
  Future<String> order({var shopData}) async{
    ...
    try{
      ...
      int minV = 100000000;
      int maxV = 999999999;
      int orderIdN = min + randomizer.nextInt(maxV - minV);
      ...
      orderData = {
        ...
        'pickup': true,
        'vehicleName': '',
        'vehicleBrand': selectedBrand != null ? selectedBrand : " ",
        'vehicleType': vehicleType,
        'address': {'address': "${currentUser.pincode} ${currentUser.city} ${currentUser.street} ${currentUser.state}", 'pincode': ''},
  ...
  Future<String> currentOrdersFetch() async{
    ...
    try{
      ...
      if(_docSnapshot.data()["order"] != null) {
        ...
        if(_docSnapshot.data()["orderHistory"]!= null) {
          orderIds2 = _docSnapshot.data()["orderHistory"];
        }
        if(orderIds2.isEmpty) {
          retVal = await detailsCurrentOrders(orderIds: orderIds, which: "current");
        } else {
          retVal = await detailsCurrentOrders(orderIds: orderIds, which: "current");
          await detailsCurrentOrders(orderIds: orderIds2, which: "past");
        }
      }
      else{
        retVal = "empty";
  ...
  Future<String> detailsCurrentOrders({List orderIds, String which}) async {
    ...
    try{
      await Future.forEach(orderIds, (element) async{
        ...
        if(which == "current") {
           _docSnapshot =  await _firestore.collection("currentOrder").doc(element).get();
        } else {
           _docSnapshot =  await _firestore.collection("orderHistory").doc(element).get();
        }
        ...
        if(which == "current") {
          currentOrders.add(
              OrderModel(
                  ...
                  empUrl: _docSnapshot.data()["empUrl"]
        ...
        allResponses.forEach((element) {
          ...
          garagesList.add(
            ...
            images: ImagesModel(
              ...
              carosuelImgs: element["garageDetails"]["images"]["carosuelImgs"],
            ),
